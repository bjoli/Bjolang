...
|------------------------------+--------------------------------------------+--------------------------------|
| =inbox-post!=, =inbox-call=  | callbacks that cannot wait: events, Kestrel | the inbox's =WhenFull= decides |
...
refuses the new item (=#f=, or a faulted =InboxFullException= for a call), which
is right when every item matters and an HTTP server answers 503. =DropOldest=
evicts the oldest instead, which is right when only the latest value matters: a
progress callback under =DropNewest= keeps the ten oldest updates and loses
100%, leaving the bar stuck at 62%. Both count what they threw away, so a
program can say "dropped 37 progress updates".
