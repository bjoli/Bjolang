...
;; every body supports def+. The first succeeding form
;; swallows the rest of the body, and Err is then returned
;; directly
(: print-key (-> (Map Keyword string) (Result string string)))
(defun (print-key myMap)
  (def+ ((Result b) (try-ref myMap :key))
        (_ (Err "Map was somehow not populated correctly")))
  (println "b is #${b}.")
  (Ok b))
...
do literal hoisting of regular expressions
...
case-of-known-constructor and dead-binding elimination would kill all allocations in loop without the jit having to do any work. 
