...
;; With nothing in that slot the missing case is rebuilt at the body's
;; type — here the Err is carried straight out, and only Option and
;; Result may
...
(defun (both-keys myMap)
  (def* ((Some a) (map-try-ref myMap :first))
        ((Some b) (map-try-ref myMap :second)))
...
(defun (receipt ls)
  (def* (total (list-foldl #(+ %1 (amount-of %2)) 0.0 ls))
        (net (/ total 1.12))
        (vat (- total net)))
